...
for dataSet = 1:length(db)
    ...
    for k=1:numFolds
        ...
        % Normalizing and selecting features
        [trainSet, testSet] = normalization(trainSet, testSet);
        [trainSet, testSet, numDim] = featureSelection(trainSet, trainTargets, testSet);
        
        prototypes = DCIACSO(trainSet, trainTargets, numDim, numCls);
    ...
    fullPath = strcat(mainPath, algPath, clfPath, resultsFolder);
