...
% Train Set is tested with a subset of features. The classifier is 5NN
    ...
    if all(solution == 0)
        ...
        knn = fitcknn(selTrSet, trainTargets);
        output = predict(knn, selTrSet);
    
        % Calculating fitness as AUCarea
        fitness = calculateAUCarea(output, trainTargets); 
    end
end